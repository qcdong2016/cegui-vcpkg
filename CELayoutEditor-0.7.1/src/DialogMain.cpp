...
#define LEFT_SCALE      wxT("Left Scale")
#define LEFT_OFFSET     wxT("Left Offset")
#define TOP_SCALE       wxT("Top Scale")
#define TOP_OFFSET      wxT("Top Offset")
#define RIGHT_SCALE     wxT("Right Scale")
#define RIGHT_OFFSET    wxT("Right Offset")
#define BOTTOM_SCALE    wxT("Bottom Scale")
#define BOTTOM_OFFSET   wxT("Bottom Offset")
...
void DialogMain::AddRealProperties(const bool mapSkins)
{
    ...
    {
		LogDebug (wxT("DialogMain::AddRealProperties - Common Property '%s' = '%s'"), StringHelper::ToWXString((*current).first).c_str(), StringHelper::ToWXString((*current).second).c_str());
    ...
    if(!m_displayOnlyCommon)
    {
        for(current = nonCommonProperties.begin(); current != nonCommonProperties.end(); ++current)
        {
            ...
            LogDebug (wxT("DialogMain::AddRealProperties - NonCommon Property '%s' = '%s'"), StringHelper::ToWXString((*current).first).c_str(), StringHelper::ToWXString((*current).second).c_str());
...
void DialogMain::ApplyFakePropertyChange()
{
	...
	else
	{
		...
		newWindowArea.d_min.d_x.d_scale = StringHelper::WXStringToFloat(m_grid->GetCellValue(1, COLUMN_VALUE));
		newWindowArea.d_min.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(2, COLUMN_VALUE));
		newWindowArea.d_min.d_y.d_scale = StringHelper::WXStringToFloat(m_grid->GetCellValue(3, COLUMN_VALUE));
		newWindowArea.d_min.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(4, COLUMN_VALUE));
		newWindowArea.d_max.d_x.d_scale = StringHelper::WXStringToFloat(m_grid->GetCellValue(5, COLUMN_VALUE));
		newWindowArea.d_max.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(6, COLUMN_VALUE));
		newWindowArea.d_max.d_y.d_scale = StringHelper::WXStringToFloat(m_grid->GetCellValue(7, COLUMN_VALUE));
		newWindowArea.d_max.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(8, COLUMN_VALUE));
...
void DialogMain::OnSelchangedLayoutTree(wxTreeEvent& event) 
{
    ...
    if (selectedWindow != m_currentWindow && selectedWindow != NULL)
    {
        ...
        wxASSERT_MSG(m_document != NULL, ASSERT_DOCUMENT_MSG);
        m_document->SelectWindow (m_currentWindow);
...
void DialogMain::SaveProperty(int row)
{
	...
	if (m_lastSelectedWindow != NULL && m_lastSelectedRow >= 0)
	{
		...
		else
		{
			...
			const wxString propertyName = m_grid->GetCellValue(m_lastSelectedRow, COLUMN_NAME);
			...
			// Set the correct font (normal if property is at default value in all elements in
			// the current selection, bold otherwise)
			SetPropertyValueFont(m_lastSelectedRow, propertyName, true);
		}
    }
}
