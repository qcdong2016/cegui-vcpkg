...
void EditorFrame::InitializeCEGUI()
{
    ...
    m_GUIRenderer = &CEGUI::OpenGLRenderer::create(CEGUI::OpenGLRenderer::TTT_NONE);
    ...
    m_GUIRenderer->getDefaultRenderingRoot().clearGeometry(CEGUI::RQ_UNDERLAY);

    // subscribe handler to render overlay items
    m_GUIRenderer->getDefaultRenderingRoot().
        subscribeEvent(CEGUI::RenderingSurface::EventRenderQueueStarted,
            CEGUI::Event::Subscriber(&EditorFrame::RQHandler,
                                     this));
}
	...
	if (GetFileNames(path, wxT("*.scheme"), filenames))
	{
		for (index=0; index<filenames.GetCount(); index++)
		{
			try
			{
				...
				(void)CEGUI::SchemeManager::getSingleton().create(StringHelper::ToCEGUIString(wxFileName(filenames[index]).GetFullName()), "schemes");
			}
...
void EditorFrame::OnResize(wxSizeEvent& WXUNUSED(event)) 
{
    ...
    if (m_GUISystem)
    {
        // Apply to CEGUI and store in INI
        m_GUISystem->notifyDisplaySizeChanged(
            CEGUI::Size(wx_static_cast(float, clientWidth),
...
//------------------------------------------------------------------------
bool EditorFrame::RQHandler(const CEGUI::EventArgs& args)
{
    using namespace CEGUI;

    if (static_cast<const RenderQueueEventArgs&>(args).queueID != RQ_UNDERLAY ||
        !m_document)
        return false;
